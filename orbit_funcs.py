...
def unpack_desig(ppd):
    ...
    # Provisional designation, for .....................(Unfinished)
        ...
        if ppd[0]=="I" or ppd[0]=="J" or ppd[0]=="K":
            ...
            pd = str((datechars.index(ppd[0])+18))+str(int(ppd[1:3])).zfill(2)+" "+ppd[3]+ppd[6]+month_order
        else:
            #print(ppd," provisional incorrect format?")
            pd = ppd
            otype="unknown"
    else:
        print(ppd," not supported!")
        pd = ppd
        otype="unknown!"
    return(pd,otype)
