...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(description='Concatenate ring32 files with MPC mmts in given subdirectory list')
    parser.add_argument('--dirlist',type=str,nargs=1,help='Comma-delimited list of subdirectories')
    ...
    # save job info table
    runfile = rundir+'mpc_makeconcat_'+str(t0)+'.fits'
    Table(jstr).write(runfile,overwrite=True)

    # Start submitting jobs
    #----------------------
    print(len(jstr)," directories of ring32 pix to concatenate")
    ...
    while eflag==0:
        for ch in chans:
            ...
            if submit_flag==1:
                ...
                dir = jstr['directory'][jsub]
                cmd = "python "+localdir+"mpc_makecat_concat.py "+str(dir)
                prt = jstr['partition'][jsub].split("_")[0]
                jstr['cmd'][jsub] = cmd
                jname = 'mpc_makeconcat_'+str(t0)+'_'+str(jcount)
