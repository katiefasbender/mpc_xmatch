...
if __name__ == "__main__":
    ...
    #---------------------------
    # Get a list of the split files in the proper directory
    nsf = len(ring32s)
    #print(splitfiles)
    # Make a table for the job information
    jstr = Table()
    jstr['ring32'] = ring32s['ring32']
    ...
    # save job info table
    runfile = rundir+'mpc_xmatch_'+str(t0)+'.fits'
    Table(jstr).write(runfile,overwrite=True)

    # Start submitting jobs
    #----------------------
    print(len(jstr)," ring32 HEALPix to xmatch between CANFind and MPC")
