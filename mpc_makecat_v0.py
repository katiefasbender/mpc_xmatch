...
# mpc_makecat.py will read in 100,000 lines from one of the
...
if __name__=="__main__":
    ...
    tner = int(sys.argv[2]) # so we can start at line zero and loop over until a specific line (ln_cnt)
    if len(sys.argv)>3: maxlines = int(sys.argv[3])
    else: maxlines = 100000 # the maximum number of lines we'll add to this table, write to this outfile
    # repos & files
    basedir = "/home/x25h971/catalogs/"
    localdir = basedir+"files/"
    listdir = basedir+"mpc/"
    outdir = listdir+"ring32/"
    outfiledir = listdir+"outfiles/checks/"
    if mode=="num":
        mpcfile = listdir+"NumObs.txt.gz"
    elif mode=="unn":
        mpcfile = listdir+"UnnObs.txt.gz"
    elif mode=="itf":
        mpcfile = listdir+"itf.txt.gz"
    elif mode=="test": mpcfile=listdir+"xaa"
    print("reading ",maxlines," from line ",tner," in mode ",mode)

    # --Read in MPC file --
    # ---------------------
    # read in maxlines, starting from tner
    w = gzip.open(mpcfile,"r")
    print("Opening MPC file ",mpcfile)
    print("starting at line ",tner)
    total_cnt = 0
    ln_cnt = 0
    for line in w:
        total_cnt+=1
        if ln_cnt<tner:
            ...
            if ln_cnt%10000==0: print("line ",ln_cnt,"/",str(tner+maxlines))
            row = read_mpc80_line(line.decode(),mode=mode)
            if row!="satellite!":
                if row[0]!=-9999:
                    mpc_mmts.add_row(row)
                    ln_cnt+=1
            # once maxlines is reached, stop reading in lines (to conserve memory)
            if len(mpc_mmts)==maxlines:
                print("done reading lines")
                break
    if len(mpc_mmts)<maxlines:
        with open(listdir+"kill_"+str(mode)+".txt","w") as killfile: pass
    # format the MPC mmts nicely, make it nice!
    mpc_good = mpc_mmts['mjd']!=-9999
    print(len(mpc_mmts[mpc_good])," measurements of ",len(np.unique(mpc_mmts[mpc_good]['name']))," tracklets from ",mpcfile)
    ...
    for r32 in ring32s_unique:
        ...
        outfile = outdir+str(r32//1000)+"/"+str(r32)+".fits"
        if os.path.exists(outfile): r32dat = Table.read(outfile)
        else: r32dat = Table()
        try:
            r32dat = vstack([r32dat,mpc_r32s])
            makedir(outdir+str(r32//1000)+"/")
            r32dat.write(outfile,overwrite=True)
            ring32s_appended.append(r32)
            #print(len(mpc_r32s)," MPC mmts written to ",outfile)
        except Exception as e: print("r32 = ",r32,e)
    nrings = len(ring32s_unique)
    nappended = len(ring32s_appended)
    with open(outfiledir+str(mode)+"_"+str(tner)+"_"+str(total_cnt)+"_"+str(nrings)+"_"+str(nappended)+".txt","w") as fl: pass
